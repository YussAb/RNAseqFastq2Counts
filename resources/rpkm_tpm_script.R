...
ftr.rpkm <- ftr.cnt %>%
  ...
  select(-cnt) %>%
  spread(sample, rpkm)
write.table(ftr.rpkm, file=paste0(output, "_rpkm.txt"), sep="\t", row.names=FALSE, quote=FALSE)
...
ftr.tpm <- ftr.cnt %>%
  ...
  select(-cnt) %>%
  spread(sample, tpm)
write.table(ftr.tpm, file=paste0(output, "_tpm.txt"), sep="\t", row.names=FALSE, quote=FALSE)
